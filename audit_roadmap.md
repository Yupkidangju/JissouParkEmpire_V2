...
| NPC Turn Lost Update | NPC 턴 루프 | 높음 | growth 내 case() UPDATE 및 범용 엔진 commit=False 차단 확인 |
...
| 보호 모드 Lost Update 방지 | ✅ | with_for_update() 락 획득 후 동시 GET/POST 요청 로컬 회귀 범위 내 확인 |
| 원자적 재시작 및 자동 복구 | ✅ | restart 단일 트랜잭션 결합 및 공원 유실 시 index/login/dashboard 자동 복구 검증 |
| Gunicorn 프로세스 락 확보 | ✅ | 비관적 락(with_for_update) 획득 및 정렬 락을 통한 우회/데드락 위험 저감 확인 |
| NPC 턴 트랜잭션 원자화 | ✅ | case() 기반 passive_growth 적용 및 commit=False 옵션 도입으로 부분 커밋 누출 차단 |
| AP 누수 차단 및 보상 트랜잭션 | ✅ | 고의적 유효성 실패 유도 시 AP 즉각 환불 복구 및 누수 0건 확인 |
| 좀비 상태 방지 및 TOCTOU 검증 | ✅ | 턴 진행 후 멸망 차단 확인 및 비관적 락 획득 후 멸망 재검증 가드 통과 |
| Cascade 연쇄 삭제 에스크로 방어 | ✅ | 상대방 공원 삭제(재시작) 시 pending 거래 및 active 밀사의 에스크로 원자적 자동 환불 확인 완료 |
| 교차 외교 중복/모순 방어 | ✅ | Canonical Ordering, ID 정렬 2중 락 및 Bulk Update 적용을 통한 교차 중복 생성 및 동맹/적대 모순 상태 저감 확인 완료 |
| NPC 턴 및 밀사 락 격리 무결성 | ✅ | 개별 트랜잭션 단위 격리 조회, Nested Savepoint 예외 가드 및 밀사 2-Way Lock을 통한 락 분실/덮어쓰기 위험 저감 |
| 교역 거절 IDOR 보안 가드 | ✅ | 타인의 교역 ID로 거절 시도 시 조건 검증 실패로 0건 업데이트 및 거절 무산 확인 |
| 시장 좀비 거래 정리 | ✅ | 발송자 멸망 시 교역 상태 pending 유지에도 공개 시장 목록에서 즉시 비노출 확인 |
| 슬로우 패스 AP 복제 Lost Update 방지 | ✅ | 턴 동기화 갭 동안 패스트 패스로 AP 소진 후 슬로우 패스 커밋 시 Stale AP 덮어쓰기 방지 확인 |
| NPC 중첩 Savepoint 깨짐 방지 | ✅ | 공격 행동 내부 commit 대신 flush 사용으로 ResourceClosedError 및 루프 무한 AP 미소모 현상 완화 확인 |
| AP 환불 정합성 유지 | ✅ | AP 환불 복구 직후 라우터 단 명시적 db.session.commit() 집행을 통한 teardown 롤백 유실 여부 확인 |
| 밀사 overcrowding Lost Update 방지 | ✅ | overcrowding 연산 진입 직전 플레이어 공원 2차 비관적 락 및 refresh 동기화를 통한 데이터 덮어쓰기 경쟁상태 차단 확인 |
| NPC 공격 락 순서 역전 데드락 완화 | ✅ | `process_npc_turn` 최상단 `with_for_update` 제거 후 공격 시 Canonical Locking으로 데드락 위험 저감 확인 |
...
| **MS 3: Actions & Modals** | - 액션 버튼 호버/클릭(active:scale-95) 애니메이션<br>- 솎아내기 Crimson 경고 점멸<br>- 정찰/침공 모달을 Combat Analyzer 테마로 조정 | 솎아내기 시 confirm 창이 정상적으로 뜨며, 모달 팝업 시 뒷배경 흐림 효과(`backdrop-filter: blur`)가 미려하게 먹히는지 검증 | 모달의 폼 전송 데이터가 `game_routes.py` 라우트 파라미터와 오차 없이 통신 |
...
| T12 | 공원 유실 자동 복구 검증 | 유저 공원을 DB에서 강제 삭제 후 `/dashboard` 또는 `/login` 진입 | 무한 리다이렉트 없이 기본 사양 공원으로 자동 재생성되는지 확인 |
| T13 | Gunicorn 교역 제한 우회 검증 | 4개 워커 기동 하에 동일 유저가 동시에 20개의 교역 생성 API 동시 호출 시도 | 10개 한도를 넘어가는 추가 등록건이 철저히 차단되고 직렬화되어 처리됨 |
| T14 | NPC 턴 예외 시 롤백 정합성 | NPC AI 행동 루프 진행 도중 임의의 예외(Exception) 유발 및 복구 | AP 감소 및 자원 변경 등의 중간 데이터가 누출 커밋 없이 롤백됨 |
| T15 | AP 누수 및 보상 트랜잭션 정합성 | `park.strike_turns = 1`(파업) 상태에서 채집/건설 등을 호출하여 고의적 유효성 실패 유도 | 행동 실행은 무산되지만 이미 consume_turn으로 선행 차감된 AP(1)가 즉시 복구(환불)되는지 확인 |
| T16 | 좀비 액션 및 TOCTOU 멸망 정합성 | 턴 소모 중 보스 HP=0 멸망 유발 또는 교역 수락 비관적 락 중 대기 중에 멸망 유도 | 턴 도중 멸망 시 즉시 차단되며, 교역 수락 및 전투 실행 도중 락 획득 후에도 무산 및 롤백되는지 확인 |
| T17 | Cascade Delete 에스크로 환불 검증 | B유저가 A유저에게 교역 제안(에스크로) 및 밀사 파견 후 A유저가 공원 삭제(/restart) 실행 | B유저가 묶어둔 거래 자원 및 파견된 성체실장이 유실 없이 B유저 공원으로 자동 환불되는지 확인 |
...
| T19 | NPC 개별 트랜잭션 격리 및 Savepoint 무결성 검증 | Gunicorn 다중 프로세스 동기화 중 NPC AI 행동 내부 예외 유발 후 락 및 턴 소모 여부 검증 | 예외 발생 시 Nested Transaction만 롤백되어 비관적 락 및 turn_count가 유지되는지 확인 |
...
| T21 | 시장 좀비 거래 정리 | 공개 거래를 등록한 유저가 멸망(is_destroyed=True)된 상태에서 타 유저의 교역 시장 페이지 진입 | is_destroyed가 True인 유저의 대기 교역은 JOIN 쿼리에 의해 시장에 노출되지 않음 |
...
| T24 | AP 환불 정합성 검증 | 채집/출산 등 행동 시도 시 고의적으로 유효성 실패를 유도하여 `refund_ap` 분기 태운 후, HTTP 리다이렉션 직전 트랜잭션 롤백 여부 검증 | 라우터 단에서 명시적 커밋을 집행하여 AP 환불 수치가 유실 없이 DB에 반영되는지 확인 |
...
| T26 | NPC 공격 락 순서 역전 데드락 완화 검증 | NPC 턴 진행 도중 공격 행동을 유발하는 동시에, 플레이어가 해당 NPC를 향해 공격/교역/외교를 동시 기동하여 DB 락 충돌 유도 | 교착 상태(Deadlock)나 DB 커넥션 풀 고갈 없이 두 요청이 Canonical Ordering 정렬에 따라 순차 처리되거나 롤백되는지 확인 |
